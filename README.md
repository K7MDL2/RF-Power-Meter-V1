...
*** V2.4 uploaded to Master Branch on 12/1/2020.   This is the first working port from the PSoC5 to Arduino Teensy 4.1. Initial testing shows the Desktop app fully working, EEPROM working and most of the Nextion display working. The Nextion Graphing display page causes a display lockup, likely due to a page number detection miss causing the CPU to get out of sync with the display.  

The OLED (SSD1306) is not working, it is still the PSoC5 verison so should remain commented out.  There are lots of warnings to sift through and cleanup and of course more testing and some fixes to do but so far the AD performance looks stable adequate using a 10K pot to similate RF detector votlage. I will know more once I see an actual RF detector connected since very small voltage changes (log scale) result in large changes when converted to Watts (linear scale). The Teensy is still only 10bit ADC but looks like the 3.3V regulator onboard results in a decently clean VRef voltage (3.3V).  Unlike the Nano and PSoC the Teensy is a 3.3 V part so have to keep that in mind when connecting peripherals.  It does accpet 5V in and can be powered from USB 5V.  The Nextion uses a 5V supply but has 3.3V tolerant TX and RX pins.  Ther Nextion firmware is unchanged and is so far independent of CPU platform.
...
Like the latest PSoC version 2.3 this has analog ports defined for 14V and HVDC, current detectror and temperature. This build has the band decoding features of hte PSoC version (Serial OTRSP and BCD input and parallel outputs, but it is not functional yet. To get the converson to Teensy going quickly I am only writing a multi-bit wide port to a single IO pin for now.  The PSoC used hardware Control and Status registers to group dispersed IO pins into a single byte for ease of programming.  Will try to group IO pins on the Teensy to avoid software mapping effort. Expect a series of rapid small changes as I work though all the subsystems and clean up warnings.
