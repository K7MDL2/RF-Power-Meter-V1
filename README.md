...
1.02 - Under dev in branch 1.02 as of May 5, 2020
   ...
   13. Likely will port this over to Cypress Semiconductor PSoc5LP which is similar cost and size but far higher AD specs with multiple reference voltage level options and programmable hardware digital and analog blocks for signal conditioning and more I/O for controlling things.  Looking at embedding these meters in my new high power RF amplifier builds, implement sequencing and fault trips. I will probably add back some sort of screen for status.  Could also just use a few LEDs.  See my Multiband LO project for a good example.
