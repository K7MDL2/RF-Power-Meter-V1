...
** Note - 8/1 - Continued debugging Aux msg and Meter band sweticign back to HF after each band change. Modified Serial parsing to look for possible BAND msg, have not seen one. Also not seeing any AUX2 messages, believe they were parser errors, now cleaning out the buffer each use, lookig for any length message to max of 16 chars.  
Working as it should now.  Tested with N1MM on 2 K3s.  One with real transverters configured up to 1296, the second with no actual transverters using various HF IFs. This similuates a radiop that does nto natively support transverters.  Set N1MM to do transverter offset for each band though 10GHz and assing to the proper IF. The Antenna Table has antennas defined on 1-10 slots for each band from HF on slot 0 (all bands comma seperated 1.8 to 28), 50, 144, 222, 432, 902, 1296, 2300, 3400, 5650, 10000 is on slot 10. This gives your Aux output on 4 bit port for values of 0-10 BCD.  Aux2 ports are provided as well in case Radio 2 is configured.  A problem I think I found is one radio with transverter offsets defined will not work with a radio with real transverters in SO2R. The transverter offset table is not assignable to a radio. This should be a feature request to the N1MM team.
