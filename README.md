...
*** V2.4 uploaded to Master Branch on 12/1/2020.   This is the first working port from the PSoC5 to Arduino Teensy 4.1. Initial testing shows the Desktop app fully working, EEPROM working, and most of the Nextion display working. The Nextion display will appear to freeze fairly often when cycling though pages.  This is due to the touch events getting missed despite the serial buffer correctly catching every event message.  The error causes the CPU to get out of sync with the display page so updates for that page do not happen. Very strange.    

The OLED (SSD1306) is not working, it is still the PSoC5 verison so should remain commented out.  There are lots of warnings to sift through and cleanup and of course more testing and some fixes to do but so far the AD performance looks stable & adequate using a 10K pot to similate RF detector voltage. I will know more once I see an actual RF detector connected since very small voltage changes (log scale) result in large changes when converted to Watts (linear scale). The Teensy is still only a 10bit ADC but looks like the 3.3V regulator onboard results in a decently clean VRef voltage (3.3V).  Unlike the Nano and PSoC the Teensy is a 3.3 V part so have to keep that in mind when connecting peripherals.  It does accept 5V in and can be powered from USB 5V.  The Nextion uses a 5V supply but has 3.3V tolerant TX and RX pins.  The Nextion firmware is unchanged and is so far independent of CPU platform.
...
This build has the band decoding features of the PSoC version Serial OTRSP and BCD input and parallel outputs.  The OTRSP Serial input decoding and display is mostly working but no IO output pins are assigned yet.  It will however change bands in the Wattmeter. You connect via a USB com port on a PC and send AUXYZZ and a \r (carriage return or CR).  Y is Radio 1 or Radio 2, ZZ is 00 to FF BCD (0 to 255).  I believe N1MM+ and other loggers would only send out 0-F BCD.  I have the band change function working when a valid AUX1XX is received (Radio 1).  It will also accept the command BANDxY where x is 1 or 2 (Radio 1 or Radio 2) and Y is 0-F representing 16 bands. It does nothing today, not sure if this is a real command in the OTRSP specs or not. 

For the Band Decoder outputs, to get the conversion to Teensy going quickly I am only writing a multi-bit value port to a single bit IO pin for now.  The PSoC used hardware Control and Status registers to group dispersed IO pins into a single byte for ease of programming.  Will try to group IO pins on the Teensy to avoid software mapping effort. Expect a series of rapid small changes as I work though all the subsystems and clean up warnings.
