...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Serial) || \
         ...
         
{
        ...
        */
        if(0u != Serial_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = Serial_GetRxBufferSize();
                CyDelay(Serial_BYTE2BYTE_TIME_OUT);
            }
            while(Serial_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  Serial_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(Serial_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(Serial_GetRxBufferSize() > 0u)
    {
        tempCount = Serial_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = Serial_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional Serial_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != Serial_PACKET_EOP)
            {
                CyDelay(Serial_WAIT_EOP_DELAY);
            }
        }
